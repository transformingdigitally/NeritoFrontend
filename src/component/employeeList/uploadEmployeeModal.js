...
const UploadEmployeeModal = ({ open, handleClose }) => {
  const translation = useTranslation();
  const downloadFile = async () => {
    await fetch(
      "https://uzs0qm0012.execute-api.us-west-1.amazonaws.com/dev/s3?key=payroll-nerito-us-west-1/csv-format/employee-csv-format.csv",
      {
        method: "GET",
      },
    )
      .then((response) => {
        response.blob().then((blob) => {
          ...
          if (navigator.msSaveBlob) {
            csvURL = navigator.msSaveBlob(csvData, "ac_template.csv");
          } else {
          ...
          tempLink.setAttribute("download", "ac_template.csv");
          tempLink.click();
        });
      })
      .catch((error) => {});
        ...
        <Box className="complistModelContainer">
          <Container className="cl-modelinner" sx={{ ...style }}>
            ...
            <Typography gutterBottom variant="h2" align="center">
              {translation.INSTRUCTION_LOADING_DATA_PAYROLL_USER}
            </Typography>
